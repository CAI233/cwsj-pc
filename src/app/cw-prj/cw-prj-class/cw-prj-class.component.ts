...
@Component({
  selector: 'app-cw-prj-class',
  templateUrl: './cw-prj-class.component.html',
  styleUrls: ['./cw-prj-class.component.css']
})
export class CwPrjClassComponent implements OnInit {
  ...
  _saveRow() {
    ...
    this.service.post('/api/busiz/cat/Cooperation/save', this.editRow).then(success => {
      if (success.code == 0) {
        this.editRow = {};
        this.load();
      }
  ...
  //删除
  _delete(data) {
    this.service.post('/api/busiz/cat/Cooperation/del', {ids:[data.cat_id]}).then(success => {
      if(success.code==0){
        this.load();
