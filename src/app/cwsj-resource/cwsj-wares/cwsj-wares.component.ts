...
@Component({
  selector: 'app-cwsj-wares',
  templateUrl: './cwsj-wares.component.html',
  styleUrls: ['./cwsj-wares.component.css']
})
export class CwsjWaresComponent implements OnInit {
  tableData: any = [];
  tableDataTree: any = [];
  editRow: any = {};
  // 实例化一个对象
  ...
  load() {
    this._loading = true;
    this.service.post('/api/busiz/cat/commodity/gettree').then(success => {
  ...
  _saveRow() {
    ...
    this.service.post('/api/busiz/cat/commodity/save', this.editRow).then(success => {
      if (success.code == 0) {
        this.editRow = {};
        this.load();
      }
  ...
  //删除
  _delete(data) {
    this.service.post('/api/busiz/cat/commodity/del', {
      mark: 'del',
      ids: [data.cat_id]
    }).then(success => {
      this.load();
