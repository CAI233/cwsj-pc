...
@Component({
  selector: 'app-cwsj-video-class',
  templateUrl: './cwsj-video-class.component.html',
  styleUrls: ['./cwsj-video-class.component.css']
})
export class CwsjVideoClassComponent implements OnInit {
  tableData: any = [];
  tableDataTree: any = [];
  editRow: any = {};
  // 实例化一个对象
  ...
  load() {
    this._loading = true;
    this.service.post('/api/busiz/cat/video/gettree').then(success => {
      this.tableData = [{
        cat_id: 0,
        cat_name: '根节点',
  ...
  _saveRow() {
    ...
    this.service.post('/api/busiz/cat/video/save', this.editRow).then(success => {
      if (success.code == 0) {
        this.editRow = {};
        this.load();
      }
  ...
  //删除
  _delete(data) {
    this.service.post('/api/busiz/cat/video/del', {
      mark: 'del',
      ids: [data.cat_id]
    }).then(success => {
      this.load();
