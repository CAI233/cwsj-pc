...
export class CwTrainClassComponent implements OnInit {
  ...
  load() {
    this._loading = true;
    this.service.post('/api/busiz/video/cat/list', { org_id: this.param.org_id }).then(success => {
      this.tableData = [{
  ...
  _saveRow() {
    ...
    this.service.post('/api/busiz/video/cat/save', this.editRow).then(success => {
      if (success.code == 0) {
        this.editRow = {};
        this.load();
      }
  ...
  //删除
  _delete(data) {
    this.service.post('/api/busiz/video/cat/del', {
      mark: 'del',
      ids: [data.cat_id]
    }).then(success => {
      this.load();
